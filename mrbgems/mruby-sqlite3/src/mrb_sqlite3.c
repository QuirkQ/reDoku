...
#include "sqlite3.h"  /* vendored amalgamation header, same dir (a gem's
                         src/ is not on the C include path; the quoted form
...
mrb_mruby_sqlite3_gem_init(mrb_state* mrb) {
  ...
  /* Renamed from "execute" (upstream): the Ruby layer in mrblib wraps this
     as SQLite3::Database#execute(sql, binds = []) returning an array of
     rows, per the M3a plan's Store API. */
  mrb_define_method(mrb, _class_sqlite3_database, "_execute", mrb_sqlite3_database_execute, MRB_ARGS_ANY());
